...
def get_capture_list(credentials, s3_client = None):
...
def get_replays_for_capture(credentials, capture_folder, s3_client = None):
  if s3_client is None:
    s3_client = boto3.client('s3', **credentials)
...
def get_capture_replay_list(credentials):
  ...
  for capture in capture_list:
    replay_list = [replay.replace(capture, "").replace("/", "") for replay in get_replays_for_capture(credentials, capture, s3_client)]
